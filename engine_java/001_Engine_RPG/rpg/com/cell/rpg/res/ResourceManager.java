...
//	--------------------------------------------------------------------------------------------------------------------
	 ...
	 * @throws Exception
	 */
	public ResourceManager(String res_root, String g2d_save) throws Exception
	{
		this.res_root	= res_root;
		this.g2d_save 	= g2d_save;
	...
	public ResourceManager(String res_root, String g2d_save, ResourceMetaData meta)  throws Exception
	{
		this.res_root	= res_root;
		this.g2d_save	= g2d_save;
		this.meta_data	= meta;
...
//	--------------------------------------------------------------------------------------------------------------------
	...
	{
		meta_data.all_scene_set	= readSets("/" + g2d_save + "/resources/scene_list.list",	SceneSet.class,		percent, 0 , 4);
		meta_data.all_actor_set	= readSets("/" + g2d_save + "/resources/actor_list.list",	SpriteSet.class,	percent, 1 , 4);
		meta_data.all_avatar_set= readSets("/" + g2d_save + "/resources/avatar_list.list",	SpriteSet.class,	percent, 2 , 4);
		meta_data.all_effect_set= readSets("/" + g2d_save + "/resources/effect_list.list",	SpriteSet.class,	percent, 3 , 4);
	}
	...
	{
		meta_data.all_icons = readIcons("/" + g2d_save + "/icons/icon.list" );
		this.all_icons = new LinkedHashMap<String, AtomicReference<BufferedImage>>();
		for (String name : meta_data.all_icons) {
			all_icons.put(name, new AtomicReference<BufferedImage>());
		}
	...
	{
		meta_data.all_sounds	= readSounds("/" + g2d_save + "/sounds/sound.list" );
		this.all_sounds = new LinkedHashMap<String, AtomicReference<ISound>>();
		for (String name : meta_data.all_sounds) {
			all_sounds.put(name, new AtomicReference<ISound>());
		}
	...
	{
		meta_data.all_npc_talks = readNpcTalks("/" + g2d_save + "/talks/talks.list" );
		this.all_npc_talks = new LinkedHashMap<String, AtomicReference<String>>();
		for (String name : meta_data.all_npc_talks) {
			all_npc_talks.put(name, new AtomicReference<String>());
		}
...
//	--------------------------------------------------------------------------------------------------------------------------------------
	...
	public <T extends RPGObject>  String toListFile(Class<T> type) 
	{
		if (type.equals(ItemProperties.class)) {
			return "/" + g2d_save + "/item_properties/item_properties.list";
		}
		else if (type.equals(Quest.class)) {
			return "/" + g2d_save + "/quests/quest.list";					
		}
		else if (type.equals(QuestGroup.class)) {
			return "/" + g2d_save + "/questgroups/questgroups.list";					
		}
		else if (type.equals(Scene.class)) {
			return "/" + g2d_save + "/scenes/scene.list";					
		}
		else if (type.equals(InstanceZone.class)) {
			return "/" + g2d_save + "/instance_zones/zones.list";					
		}
		else {
			String type_name = type.getSimpleName().toLowerCase();
			return "/" + g2d_save + "/objects/" + type_name + ".obj" + "/" + type_name + ".list";
		}
	}
