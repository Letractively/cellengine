	...
	private class TextLine 
	{
		...
		void render(Graphics2D g, int shadow_x, int shadow_y, float shadow_alpha, int shadow_color)
		{
			...
			try {
				...
				text_layout.draw(((AwtGraphics2D)g).g2d, offsetx, offsety);
			} finally {
				g.translate(-tx, -ty);
			}
		}
...
//						g2d.fillRect(0, 0, width, height);
	...
	private class TextChanges
	{
		/** 强行刷新数据 */
		boolean				force	= false;
		String 				text	= AwtMultiTextLayout.this.text;
		AttributedString 	atext 	= AwtMultiTextLayout.this.attr_text;
		int 				width	= AwtMultiTextLayout.this.width;
		int					space	= AwtMultiTextLayout.this.line_space;
