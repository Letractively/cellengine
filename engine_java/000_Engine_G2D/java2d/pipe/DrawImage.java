...
public class DrawImage implements DrawImagePipe
{
	...
	if (sData == null) {
	    ...
	    sData = SurfaceData.getSourceSurfaceData(img, sg.surfaceData,
						     sg.imageComp,
						     bgColor, true);
	    if (sData == null) {
	...
	if (helper == null) {
	     ...
	     */
	    int type = ((sData.getTransparency() == Transparency.OPAQUE)
	...
	SurfaceType dstType = sg.surfaceData.getSurfaceType();
        ...
        if (sg.compositeState <= sg.COMP_ALPHA) {
            ...
            if (maskblit.getNativePrim() != 0) {
                ...
                helper.Transform(maskblit, sData, sg.surfaceData,
                                 sg.composite, clip,
                                 itx, interpType,
                                 sx1, sy1, sx2, sy2,
                                 dx1, dy1, dx2, dy2,
	...
	BufferedImage tmpimg = new BufferedImage(dx2-dx1, dy2-dy1,
						 BufferedImage.TYPE_INT_ARGB);
	SurfaceData tmpData = SurfaceData.getDestSurfaceData(tmpimg);
	...
	helper.Transform(tmpmaskblit, sData, tmpData,
	...
	for (int y = edges[0]; y < edges[1]; y++) {
	    ...
	    if (relx1 >= relx2) {
            ...
            if (maskblit != null) {
		maskblit.MaskBlit(tmpData, sg.surfaceData,
				  sg.composite, clip,
				  relx1, y,
				  dx1+relx1, dy1+y,
				  relx2 - relx1, 1,
		...
		blit.Blit(tmpData, sg.surfaceData,
                          sg.composite, clip,
                          relx1, y,
                          dx1+relx1, dy1+y,
                          relx2 - relx1, 1);
