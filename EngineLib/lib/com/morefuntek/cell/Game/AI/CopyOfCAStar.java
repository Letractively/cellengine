package com.morefuntek.cell.Game.AI;
...
public class CopyOfCAStar extends CObject {
	...
	public CopyOfCAStar(CMap map){
		Map = map;
		
		AllNode = new MapNode[Map.getHCount()][Map.getWCount()];
	...
	public void findPath(int sx,int sy,int dx,int dy,int[] xpath,int[] ypath){
...
//				/*
				...
				for(int i=0;i<near.length;i++){
					if( curSrcNode.Y+near[i][1] < 0 || curSrcNode.Y+near[i][1] > Map.getHCount()-1 || //
						curSrcNode.X+near[i][0] < 0 || curSrcNode.X+near[i][0] > Map.getWCount()-1 ){ //
						continue;
					}
					...
					if( Map.getFlag(block.X,block.Y)!=0 ||
						Math.abs(block.Z - curSrcNode.Z)>2 ){
						continue;
					}
				...
				if( curSrcNode.X == dx && curSrcNode.Y == dy ){ //
					...
					for(int i=0;i<Map.getHCount()*Map.getWCount();i++){
						println(" X=" + curSrcNode.X + " Y=" + curSrcNode.Y);
...
//							
					...
					xpath = new int[X.size()];
					ypath = new int[Y.size()];
					for(int i=0;i<X.size();i++){
						xpath[i] = ((Integer)X.elementAt(i)).intValue();
						ypath[i] = ((Integer)Y.elementAt(i)).intValue();
					}
