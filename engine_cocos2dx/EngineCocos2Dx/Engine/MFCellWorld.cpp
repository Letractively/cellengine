...
// 	}
	...
	void CCellWorldLayer::removeWorldChild(CCNode* c)
	{
		CCNode::removeChild(c, true);
		//vector<CCNode*>::iterator it = m_childs.
		//m_childs.erase(it);
	}
	void CCellWorldLayer::addWorldChild(CCNode* c)
	{
		CCNode::addChild(c);
		//m_childs.push_back(c);
	}

	void CCellWorldLayer::moveWorldChild(CCNode* c, float x, float y)
	{
// 		c->+= x;
// 		c->setx
// 		c->setPosition(
...
// 			);
	...
	void CCellWorldLayer::visit()
	{
		...
		vector<CCNode*>::iterator it = m_childs.begin();
