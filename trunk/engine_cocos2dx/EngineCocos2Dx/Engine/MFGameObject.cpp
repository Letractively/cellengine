...
//		Frames[SubIndex].push_back(SubIndex);
	...
	void CCollides::render(Graphics2D *g, int index, Color const &color)
	{
		for(int i=Frames[index].size()-1;i>=0;i--){
			cds[Frames[index][i]].render(g, 0, 0, color);
		}
	}
...
//	Frames = new short[partCount][];
	...
	CAnimates::CAnimates(int partCount, ITiles *tils)
	{
		tiles		= tils;
		SubCount	= partCount;
		SubIndex	= 0;
...
//		Frames[SubIndex].push_back(SubIndex);
	...
	{
		int left	= MF_INT32_MAX;
		int right	= MF_INT32_MIN;
		int top		= MF_INT32_MAX;
		int bottom	= MF_INT32_MIN;
	 ...
	 */
	void CAnimates::render(Graphics2D *g, int index)
	{
		tiles->renderBegin(g);
		for (int i=Frames[index].size()-1; i>=0; --i) {
