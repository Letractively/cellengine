...
namespace mf
{
	...
	bool Math::moveTo(Vector2D &v, float x, float y, float distance)
	{
		float ddx = x - v.x;
		float ddy = y - v.y;
		if (MF_ABS(ddx) < distance && MF_ABS(ddy) < distance) {
			v.x = x;
			v.y = y;
			return true;
		} else {
	...
	float Math::getDistance4F(float x1, float y1, float x2, float y2)
	{
		float r1 = x1-x2;
		float r2 = y1-y2;
	...
	float Math::getDistanceV(Vector2D const &v)
	{
		return getDistance4F(0, 0, v.x, v.y);
	}
