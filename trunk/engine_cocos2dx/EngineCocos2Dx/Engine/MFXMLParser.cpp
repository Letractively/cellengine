...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	XMLNode* XMLNode::findChild(string const &name)
	{
		for (std::vector<XMLNode*>::iterator it = childs.begin(); it != childs.end(); ++it) {
			XMLNode *child = (*it);
			if (stringEquals(child->name, name)) {
				return child;
			}
		}
		return NULL;
	...
	bool XMLNode::isAttribute(string const &name)
	{
		map<string, string>::const_iterator it = attributes.find(name);
	...
	std::string	XMLNode::getAttribute(std::string const &name)
	{
	...
	float XMLNode::getAttributeAsFloat(std::string const &name)
	{
		return stringToFloat(attributes[name]);
	}
	bool XMLNode::getAttributeAsBool(std::string const &name)
	{
		return stringToBool(attributes[name]);
	}

	u32 XMLNode::getAttributeAsHexU32(std::string const &name)
	{
		return stringHexToU32(attributes[name]);
	}
	...
	void XMLNode::setValue(string const& value)
	{
		this->value = value;
	}

	void XMLNode::setAttribute(string const &key, string const& value)
	{
	...
	void parseInternal(TiXmlElement *rootElement, XMLNode* rootNode)
	{
		...
		while ( rootAttr ) {
			...
			rootNode->setAttribute(attrKey, attrValue);
