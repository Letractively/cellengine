...
namespace Cell.AppMusicEditor 
{
	...
	public class Form7 : System.Windows.Forms.Form 
	{
        ...
        private BMSCanvas[] srcCanvas;
        private BMSCanvas desCanvas;
		...
		public Form7(string file,int lineCount)
		{
			//
			// Windows 窗体设计器支持所必需的
			//
		...
		private void Form2_Load(object sender, System.EventArgs e)
		{
            try
            {
                ...
                srcCanvas = new BMSCanvas[bms.Events.Length];
                for (int t = 0; t < srcCanvas.Length; t++)
                {
                    srcCanvas[t] = new BMSCanvas(bms,bms.Events[t],bms.keyBPM, LineCount);
                    srcCanvas[t].LostScope = int.MaxValue;
                    srcCanvas[t].BuffSize = bms.Events[t].Count;
                }
                ...
                desCanvas = new BMSCanvas(bms, bms.keyPlayer, bms.keyBPM, LineCount);
                desCanvas.LostScope = int.MaxValue;
                desCanvas.BuffSize = bms.keyPlayer.Count;
                ...
                {
                    tracks[i] = new TreeNode("Track " + i + " (" + bms.Events[i].Count + ")", 2, 2);
                }
        ...
        private void pictureBox1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (desCanvas != null && soundman1 != null)
            {
                ...
                desCanvas.UpdatePosition((int)(soundman1.CurrentPosition * 1000));

                desCanvas.ScreenHeight = wh;
                desCanvas.Update(desCanvas.Position());

                int[] NoteLine = desCanvas.getFullNoteLine();
                for (int l = 0; l < NoteLine.Length; l++)
                {
                    // draw full note line
                    if (wh - NoteLine[l] > 0 && wh - NoteLine[l] < wh)
                    {
                        g.DrawLine(
                        ...
                        g.DrawString(
                                (desCanvas.getFullNoteCount() + l) + "",
                                this.Font,
                                Brushes.Gray,
                                1,
                                wh - NoteLine[l] - this.Font.Height
                                );
                    }
                }
                ...
                for (int i = 0; i < Pos.Length; i++)
                {
                    ...
                    for (int j = 0; j < Pos[i].Length; j++)
                    {
                        ...
                        {
                            if (((EventBMS)desCanvas.Events[Pos[i][j][1]]).dragged == true)
                            {
                                tiles2.render(g,
                                    ((EventBMS)desCanvas.Events[Pos[i][j][1]]).track,
                                    1 + i * (tile_w + 1),
                            ...
                            if (des_down)
                            {
                                ((EventBMS)desCanvas.Events[Pos[i][j][1]]).dragged = false;
                                if (SelectNote<0 && des_rect.IntersectsWith(note))
                                {
                                    SelectNote = Pos[i][j][1];
                                    ((EventBMS)desCanvas.Events[Pos[i][j][1]]).dragged = true;
                                }
                            }
                            ...
                            if (des_hold)
                            {
                                if (((EventBMS)desCanvas.Events[Pos[i][j][1]]).dragged == true)
                                {
                                    ...
                                    {
                                        ((EventBMS)desCanvas.Events[Pos[i][j][1]]).pos = (byte)(des_dx / (tile_w + 1));
                                    }
                                }
                            }
                            ...
                            {
                                if (((EventBMS)desCanvas.Events[Pos[i][j][1]]).selected == false)
                                {
                                    
                                    if (((EventBMS)desCanvas.Events[Pos[i][j][1]]).used == true)
                                    {
                                        ((EventBMS)desCanvas.Events[Pos[i][j][1]]).used = false;
                                    }
                                    else
                                    {
                                        ((EventBMS)desCanvas.Events[Pos[i][j][1]]).used = true;
                                    }
                                }
                                ((EventBMS)desCanvas.Events[Pos[i][j][1]]).selected = true;
                            }
                            else
                            {
                                ((EventBMS)desCanvas.Events[Pos[i][j][1]]).selected = false;
                            }
                        }
                    }
                ...
                {
                    if (((EventBMS)desCanvas.Events[i]).used == false)
                    {
                        if (((EventBMS)desCanvas.Events[i]).track == ChannelIndex)
                        {
        ...
        private void pictureBox2_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (desCanvas != null && soundman1 != null)
            {
                ...
                srcCanvas[ChannelIndex].ScreenHeight = wh;
                srcCanvas[ChannelIndex].Update(desCanvas.Position());
                ...
                for (int l = 0; l < NoteLine.Length; l++)
                {
                    ...
                    if (wh - NoteLine[l] > 0 && wh - NoteLine[l] < wh)
                    {
                        ...
                        g.DrawString(
                                (srcCanvas[ChannelIndex].getFullNoteCount() + l) + "",
                                this.Font,
                                Brushes.Gray,
                                1,
                                wh - NoteLine[l] - this.Font.Height
                ...
                for (int i = 0; i < Pos.Length; i++)
                {
                    ...
                    if (treeView1.SelectedNode.Parent != null)
                    {
                        ...
                        for (int j = 0; j < Pos[i].Length; j++)
                        {
                            ...
                            {
                                if (((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]).used == true)
                                {
                                    tiles2.render(g,
                                        ((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]).track,
                                        1 + i * (tile_w + 1),
                                        pictureBox2.Height - Pos[i][j][0] - tile_h + 1
                                        );
                                }
                                ...
                                if (src_hold && src_rect.IntersectsWith(note))
                                {
                                    if (((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]).selected == false)
                                    {
                                        if (((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]).used == true)
                                        {
                                            ((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]).used = false;
                                        }
                                        else
                                        {
                                            desCanvas.Events.Add((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]);
                                            desCanvas.BuffSize = desCanvas.Events.Count;
                                            ((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]).used = true;
                                        }
                                    }
                                    ((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]).selected = true;
                                }
                                else
                                {
                                    ((EventBMS)srcCanvas[ChannelIndex].Events[Pos[i][j][1]]).selected = false;
                                }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                if (((EventBMS)srcCanvas[ChannelIndex].Events[i]).used == false)
                {
                    ((EventBMS)srcCanvas[ChannelIndex].Events[i]).used = true;
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < desCanvas.Events.Count; i++)
            {
                if (((EventBMS)desCanvas.Events[i]).track == ChannelIndex)
                {
                    ((EventBMS)desCanvas.Events[i]).used = false;
                    desCanvas.Events.RemoveAt(i);
                    desCanvas.BuffSize = desCanvas.Events.Count;
                    i--;
                }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                ((EventBMS)desCanvas.Events[0]).used = false;
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            for (int t = 0; t < srcCanvas.Length; t++)
            {
                ...
                {
                    if (((EventBMS)srcCanvas[t].Events[i]).used == false)
                    {
                        ((EventBMS)srcCanvas[t].Events[i]).used = true;
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            //del near
            //
            EventComparerBMS ec = new EventComparerBMS();
            ...
            while (i < desCanvas.Events.Count && j < desCanvas.Events.Count)
            {
                ...
                if (Math.Abs(((EventBMS)desCanvas.Events[i]).time - ((EventBMS)desCanvas.Events[j]).time) < Math.Abs(this.numericUpDown1.Value))
                {
                    ((EventBMS)desCanvas.Events[i]).used = false;
                    desCanvas.Events.RemoveAt(i);
                    desCanvas.BuffSize = desCanvas.Events.Count;
        ...
        private void delThisLineNearToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            while (i < desCanvas.Events.Count && j < desCanvas.Events.Count)
            {
                if (((EventBMS)desCanvas.Events[i]).pos % desCanvas.GetLineCount() != line)
                {
                    i++;
                    continue;
                }
                if (((EventBMS)desCanvas.Events[j]).pos % desCanvas.GetLineCount() != line) 
                { 
                    j++;
                    continue;
                }
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (((EventBMS)srcCanvas[ChannelIndex].Events[i]).used == false &&
                    ((EventBMS)srcCanvas[ChannelIndex].Events[i]).pos % srcCanvas[ChannelIndex].GetLineCount() == line)
                {
                    ((EventBMS)srcCanvas[ChannelIndex].Events[i]).used = true;
                    desCanvas.Events.Add(srcCanvas[ChannelIndex].Events[i]);
                    desCanvas.BuffSize = desCanvas.Events.Count;
        ...
        private void delThisLineToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (((EventBMS)desCanvas.Events[i]).pos % desCanvas.GetLineCount() != line ||
                    ((EventBMS)desCanvas.Events[i]).track != ChannelIndex)
                {
                    i++;
                    continue;
                }
